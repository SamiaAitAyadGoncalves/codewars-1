...
# If we suppose that A1 is close enough to the curve, the same reasoning as
# for the point A1 above can be used for other points. After several steps,
# a polygonal curve A0, A1, .. , An is computed. The error between the 2
# curves can be small if the step is small.
#
# We define points A0, A1, A2, ... , An whose x-coordinates are x0, x1 , ... ,
# xn and y-coordinates are such that yk+1 = yk + f(xk, yk) * h where h is the
# common step. If T is the length xn - x 0 we have h = T/n.
#
# We will take as an example the differential equation:
#
# dy/dt = 2 - exp(-4t) - 2y with x0 = 0, y0 = 1, T = 1.
#
# We know that an exact solution is y = 1 + 0.5exp(-4t) - 0.5exp(-2t). For
# each xk we are able to calculate the yk as well as the values zk of the
# exact solution.
#
# Our task is, for a given number n of steps, to return the mean
# (truncated to 6 decimals) of the relative errors between the yk of
# the n+1 points Ak of our polygonal line and the n+1 zk of our exact solution.
# For that we can use:
#
...
def ex_euler(n):
    ...
    for i in range(1, n+1):
        z.append(1 + 0.5*math.exp(-4*i*h) - 0.5*math.exp(-2*i*h))
        y.append(y[-1] + f(x[-1], y[-1]) * h)
        x.append(i*h)

    A = [(abs(y[i] - z[i]) / z[i]) for i in range(n+1)]
    return math.trunc((sum(A) / (n + 1))*1000000)/1000000
